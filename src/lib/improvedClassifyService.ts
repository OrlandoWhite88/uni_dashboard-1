  ...
  continue: (answer: string): ClassificationResponse => {
    ...
    if (answer.toLowerCase().includes("cotton")) {
      ...
      return {
        ...
        full_path:
          "Section VII > Chapter 39 > Heading 3926 > Subheading 3926.90 > 3926.90.99",
      };
    } else {
      return {
        ...
        full_path:
          "Section XVI > Chapter 84 > Heading 8479 > Subheading 8479.89 > 8479.89.99",
      };
    }
  },
};
  ...
  for (const approach of approaches) {
    try {
      ...
      try {
        ...
        debug(
          `${approach.name} response is not JSON, treating as code:`,
          responseText
        );
...
export async function continueClassification(
  ...
  try {
    ...
    return {
      ...
      clarification_question: {
        question_text:
          "There was an error continuing the classification. Would you like to try again?",
        options: ["Try Again", "Cancel"],
      },
    };
  }
