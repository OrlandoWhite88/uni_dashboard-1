...
const BatchClassify: React.FC<BatchClassifyProps> = ({ csvFile }) => {
  ...
  const products = useMemo(() => {
    // If no file content, return empty array
    if (!csvFile) return [];
  ...
  const startClassifyingAllProducts = async () => {
    ...
    if (!usageRecorded) {
      console.error('Failed to record batch processing usage');
      return;
    }
  ...
  // If no products, show a prompt to add them
  if (!csvFile || products.length === 0) {
    return (
      <div className="text-center py-8">
        <FileText className="mx-auto h-12 w-12 text-muted-foreground mb-4" />
        <h3 className="text-lg font-medium mb-2">No Products to Classify</h3>
        <p className="text-muted-foreground">
          Upload a CSV file or enter product descriptions to get started with batch classification.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {showUpgradeMessage && (
        <div className="glass-card p-6 rounded-xl border border-amber-400">
